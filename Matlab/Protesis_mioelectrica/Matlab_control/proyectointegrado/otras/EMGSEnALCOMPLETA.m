...
[rango,tdetX]=findpeaks(signal);
rango=sort(rango,'descend');
for i=1:length(signal)
    if signal(i,1)==rango(1,1)
       c11=i;
    elseif signal(i,1)==rango(length(rango)-46,1)
       c33=i;
    end
end
hold on;
plot(t(c11),rango(1),'*g',t(c33),rango(length(rango)-46),'*g')

disp ('rango voltaje picos:')
disp(rango(1,1))
disp(rango(length(rango)-96,1))
...
disp(f(c1))
disp(f(c2))
disp ('LAS MAXIMAS POTENCIAS:')
disp(npontenciaY(1,1))
disp(npontenciaY(2,1))
disp(npontenciaY(3,1))
