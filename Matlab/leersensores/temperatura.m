...
t=zeros(1,1000); %vector donde se guardan los datos
...
while contador_muestras<=numero_muestras
    ...
    t(contador_muestras)=(valor_humedad(1)*5)/1024;
    plot(contador_muestras,t(contador_muestras),'X-r');grid on; hold on;
    title('% VOLTAJE VS N° DATOS ')
    xlabel ('N° de datos')
    ylabel ('Voltaje(V)')
    subplot(2,2,2)
    a=100*((t(contador_muestras)-0.958)/0.0307) ;
